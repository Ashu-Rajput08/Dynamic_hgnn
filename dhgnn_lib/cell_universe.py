...
the raw dataset or the two pre-existing lookup tables:
  ...
  - anastomosis_lookup_table.csv  (old/new_cluster_members, pipe separated)
"""
...
def build_cell_universe(raw_dir: str, output_dir: str) -> List[str]:
    ...
    anas = pd.read_csv(os.path.join(output_dir, "anastomosis_lookup_table.csv"))
    for col in ("old_cluster_members", "new_cluster_members"):
        for cell_str in anas[col].dropna().astype(str):
            names.update(_split_members(cell_str))
