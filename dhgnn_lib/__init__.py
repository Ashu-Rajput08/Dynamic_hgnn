...
Modules:
    ...
    hyperedges      - builders for spatial/lineage/functional/anastomosis
                       hyperedges + writers for the new lookup tables
    incidence       - HyperedgeSet / HyperedgeBatch + H_aug(t) assembly
    labels          - fate / alignment / anastomosis label tensors
    sparse_ops      - scatter_sum/mean/max/softmax (pure PyTorch)
    hypersagnn      - Hyper-SAGNN V2E2V convolution layer
    dhgnn_model     - DynamicHypergraphStack + DHGNNModel (3 heads)
    training        - combined loss, metrics, optimizer/scheduler, train loop
