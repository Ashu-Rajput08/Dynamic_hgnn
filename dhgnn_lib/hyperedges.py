"""Builders for the four hyperedge types that make up ``H_aug(t)``.
...
so that, alongside the pre-existing ``functional_lookup_table.csv`` and
``anastomosis_lookup_table.csv``, ``output_tables/`` contains all four
hyperedge sources needed to assemble ``H_aug(t)``.
"""
...
def build_spatial_lookup_table(
    ...
    members, cluster_size)``. Noise points (label == -1) are dropped, mirroring
    the convention used to build ``anastomosis_lookup_table.csv``.
    """
...
def recluster_spatial_hyperedge_set(
    ...
    """Rebuild spatial hyperedges by clustering learned embeddings.

    Used by the DHGNN "dynamic re-clustering" step: between layers, DBSCAN is
    re-run on the (detached) node embeddings of cells present at time ``t`` to
    discover new spatial communities directly from the learned
    representations, rather than from raw (x, y, z) coordinates.

    Args:
        embeddings: (N, D) node embedding tensor (any device).
        present_mask: (N,) bool tensor, True for cells present at time t.
        universe: the cell universe (for index -> name, though indices are
            already aligned, so this is only used for length checks).
    ...
    if len(present_idx) < min_samples:
        return HyperedgeSet.empty()
    ...
    for cluster_id in sorted(set(labels)):
        if cluster_id == -1:
            continue
        members = present_idx[labels == cluster_id]
        if len(members) < 2:
            continue
        for m in members:
...
# ---------------------------------------------------------------------------
# Anastomosis hyperedges (dynamic, from existing anastomosis_lookup_table.csv)
# ---------------------------------------------------------------------------

def anastomosis_hyperedge_set(anastomosis_table: pd.DataFrame, t: int, universe: CellUniverse) -> HyperedgeSet:
    """Hyperedges for community-switch events at time ``t``.

    Each row of ``anastomosis_lookup_table.csv`` with ``t_switch == t``
    contributes its ``new_cluster_members`` set as a hyperedge (the community
    the switching cell joins). Duplicate member sets at the same timepoint are
    de-duplicated so each distinct destination community is one hyperedge.
    """
    sub = anastomosis_table[anastomosis_table["t_switch"] == t]
    seen = set()
    member_lists = []
    for m in sub["new_cluster_members"]:
        members = tuple(_split_members(m))
        if members in seen or len(members) < 2:
            continue
        seen.add(members)
        member_lists.append(list(members))

    weights = [1.0] * len(member_lists)
    return _edges_from_member_lists(member_lists, weights, universe, min_size=2)
...
class HyperedgeStore:
    """Loads all four hyperedge sources and assembles ``H_aug(t)`` on demand."""
    ...
    def __init__(self, raw_dir: str, output_dir: str, universe: CellUniverse):
        ...
        self.anastomosis_table = pd.read_csv(os.path.join(output_dir, "anastomosis_lookup_table.csv"))
    ...
    def anastomosis(self, t: int) -> HyperedgeSet:
        return anastomosis_hyperedge_set(self.anastomosis_table, t, self.universe)
